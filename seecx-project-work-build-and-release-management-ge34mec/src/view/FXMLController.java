...
public class FXMLController {
	...
	private void submitNameButtonClicked(ActionEvent event) {
		...
		if (controller.initPlayer(name)) {
        	...
        	alert.setTitle("Wrong or empty name format");
        	alert.setContentText("The name you entered is either incorrect or missing!");
        	alert.showAndWait();
		}
	}
	...
	private void levelChosenButtonClicked(ActionEvent event) {
		...
		/*
		 * try { Parent root =
		 * FXMLLoader.load(getClass().getResource("LevelLayout.fxml"));
		 * 
		 * if (levelScene == null) this.levelScene = new Scene(root);
		 */

		levelScene.setOnKeyPressed(keyListener);
		/*
		 * stage.setScene(levelScene); stage.show(); } catch (IOException e) {
		 * e.printStackTrace(); }
		 */
	}
